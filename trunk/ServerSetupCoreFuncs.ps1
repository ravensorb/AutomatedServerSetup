...
function Execute-ConfigureDNS {
	...
	[string] $domainName = $($xmlSettings.configuration.domain.name)

	$osDetails = gwmi win32_operatingsystem
	
	Write-LogMessage -level 1 -msg "Configuring DNS Records"
	foreach ($record in $xmlSettings.configuration.dns.record) {
			...
			switch ($record.type) {
				...
				"cname" {
					$rrData = $($record.name);
					if ($rrData -like "*.$domainName") { $rrData = "$rrData.$domainName" }
...
function Execute-DNSCreateA {
	...
	Write-LogMessage -level 1 -msg "Checking for existing of DNS Record: $nodeName [Create: $create]"
